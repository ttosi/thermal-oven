...
TemperatureSensor::TemperatureSensor(uint8_t sckPin, uint8_t csPin, uint8_t soPin) : thermocouple_(sckPin, csPin, soPin)
{
}
...
{
    if (now - startedAtMs_ < Config::SENSOR_STARTUP_DELAY_MS || now - lastSampleAtMs_ < Config::TEMPERATURE_SAMPLE_MS)
    {
        return;
    }
