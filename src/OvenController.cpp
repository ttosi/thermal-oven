...
OvenController::OvenController()
    : temperatureSensor_(Config::PIN_THERMOCOUPLE_SCK, Config::PIN_THERMOCOUPLE_CS, Config::PIN_THERMOCOUPLE_SO),
      heater_(Config::PIN_SSR), ui_(Config::PIN_ENCODER_CLK, Config::PIN_ENCODER_DT, Config::PIN_ENCODER_SWITCH),
...
void OvenController::update()
{
    ...
    ui_.render(temperatureSensor_.temperatureC(), targetC_, uiStatus(), heater_.isHeaterOn(), now);
    logStatus(now);
}
...
void OvenController::handleInput(uint32_t now)
{
    ...
    if (state_ == State::Off && temperatureSensor_.isValid() && !temperatureSensor_.isStale(now))
    {
...
void OvenController::evaluateSafety(uint32_t now)
{
    ...
    if (temperatureSensor_.isValid() && temperatureSensor_.temperatureC() >= Config::MAX_SAFE_C)
    {
